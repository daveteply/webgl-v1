...
import { TextManagerService } from '@rikkle/graphics';
...
import { PostProcessingManagerService } from '@rikkle/graphics';
...
export class GameContainer implements OnInit, AfterViewInit, OnDestroy {
  @ViewChild('gameCanvas')
  gameCanvas!: ElementRef<HTMLCanvasElement>;

  private _splashTimeouts: ReturnType<typeof setTimeout>[] = [];
  ...
  ngAfterViewInit(): void {
    ...
    const t1 = setTimeout(() => {
      ...
      const t2 = setTimeout(() => {
        ...
        this._splashTimeouts.push(t3);
      }, 2000);
      this._splashTimeouts.push(t2);
    }, 150);
    this._splashTimeouts.push(t1);
  }

  ngOnDestroy(): void {
    for (const id of this._splashTimeouts) {
      clearTimeout(id);
    }
    this._splashTimeouts = [];
  }
